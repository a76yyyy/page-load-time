try { importScripts('browser-polyfill.min.js'); } catch (e) {
  console.error('[DEBUG] ❌ 加载 browser-polyfill 失败:', e);
}
try { importScripts('storage-manager.js'); } catch (e) {
  console.error('[DEBUG] ❌ 加载 storage-manager 失败:', e);
}
...
console.log('[DEBUG] 🔧 检查 PageLoadStorageManager 类:', typeof PageLoadStorageManager);
if (typeof PageLoadStorageManager === 'undefined') {
  console.error('[DEBUG] ❌ PageLoadStorageManager 类不存在！');
}
...
storageManager.init().then(() => {
  storageReady = true;
  console.log('[DEBUG] ✅ 存储管理器初始化完成');
  // 定期清理过期数据（每 30 分钟）
  setInterval(() => {
    storageManager.cleanupOldData();
  }, 30 * 60 * 1000);
}).catch(error => {
  console.error('[DEBUG] ❌ 存储管理器初始化失败:', error?.message || String(error), error?.stack);
});
    ...
    if (details.ip && details.tabId === tabId) {
      ...
      if (storageReady) {
        storageManager.saveIPData(details.url, details.ip, tabId).catch(error => {
          console.error(`[DEBUG] ❌ 保存 IP 数据失败:`, error?.message || String(error), error?.stack);
        });
      }
    } else {
...
browser.runtime.onMessage.addListener((request, sender) => {
  ...
  if (request.action === 'getIPData' && sender.tab) {
    return (async () => {
      try {
        ...
        console.error('[DEBUG] ❌ 获取 IP 数据失败:', error?.message || String(error), error?.stack);
        return {};
      }
    })();
  }
  ...
  if (request.timing) {
    ...
    (async () => {
      try {
        console.log(`[DEBUG] 📊 收到性能数据: Tab ${sender.tab.id}, duration: ${request.timing.duration}ms`);
        if (storageReady) {
          // 保存到 IndexedDB
          await storageManager.savePerformanceData(sender.tab.id, request.timing);
          console.log(`[DEBUG] 💾 性能数据已保存到 IndexedDB: Tab ${sender.tab.id}`);

          // 验证数据是否真的保存了
          const saved = await storageManager.getPerformanceData(sender.tab.id);
          if (saved) {
            console.log(`[DEBUG] ✅ 验证成功: 数据已在 IndexedDB 中`);
          } else {
            console.error(`[DEBUG] ❌ 验证失败: 数据未在 IndexedDB 中找到`);
          }
        } else {
          console.warn(`[DEBUG] ⚠️ 存储管理器未就绪，无法保存性能数据`);
        }
        ...
        console.error('[DEBUG] ❌ 保存性能数据失败:', error?.message || String(error), error?.stack);
      }
    })();
    // 不需要响应,返回 undefined
    return;
...
browser.tabs.onRemoved.addListener(tabId => {
  ...
  if (storageReady) {
    storageManager.deleteTabData(tabId).catch(error => {
      console.error(`[DEBUG] ❌ 清理 Tab ${tabId} 的数据失败:`, error?.message || String(error), error?.stack);
    });
  }
});
